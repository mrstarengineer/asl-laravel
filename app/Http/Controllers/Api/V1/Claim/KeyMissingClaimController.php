...
class KeyMissingClaimController extends Controller
{
    private $service;
    ...
    {
        $data = $this->service->all( array_merge( $request->all(), [ 'type' => ClaimType::KEY_MISSING_CLAIM ] ) )->toArray();
    ...
    public function store ( Request $request )
    {
        ...
        DB::beginTransaction();
        try {
            $data = $this->service->store( array_merge( $request->all(), [ 'type' => ClaimType::KEY_MISSING_CLAIM ] ) );
            DB::commit();

            debug_log( "Key Missing Claim created successfully!", $data );

            return api( $data )->success( 'Key Missing Claim Created successfully!', Response::HTTP_CREATED );
        } catch ( \Exception $e ) {
            debug_log( "Key Missing Claim create failed!", $e->getTrace() );
            DB::rollback();
    ...
    public function update( $id, Request $request ): \Illuminate\Http\JsonResponse {
        ...
        try {
            ...
            debug_log( "Key Missing Claim updated successfully!", $data );

            return api( $data )->success( 'Key Missing Claim Updated successfully!', Response::HTTP_CREATED );
        } catch ( \Exception $e ) {
            debug_log( "Key Missing Claim update failed!", $e->getTrace() );
            DB::rollback();
    ...
    public function destroy( $id ): \Illuminate\Http\JsonResponse {
        try {
            ...
            debug_log( "Key Missing Claim deleted successfully!", $data );

            return api( $data )->success( 'Key Missing Claim Deleted Successfully!' );
        } catch ( \Exception $e ) {
            debug_log( "Key Missing Claim deletion failed!", $e->getTrace() );
    ...
    public function downloadPhotos ( $id, Request $request )
    {
        try {
            ...
            $allImages = ClaimImage::where( ['key_missing_claim_id' => $id, 'type' => $type] )->get();
