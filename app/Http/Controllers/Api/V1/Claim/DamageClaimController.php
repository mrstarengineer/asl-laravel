...
class DamageClaimController extends Controller
{
    ...
    {
        $data = $this->service->all( array_merge( $request->all(), [ 'type' => ClaimType::DAMAGE_CLAIM ] ) )->toArray();
    ...
    public function store ( Request $request )
    {
        ...
        try {
            ...
            debug_log( "Vehicle Claim created successfully!", $data );

            return api( $data )->success( 'Vehicle Claim Created successfully!', Response::HTTP_CREATED );
        } catch ( \Exception $e ) {
            debug_log( "Vehicle Claim create failed!", $e->getTrace() );
            DB::rollback();
    ...
    public function update( $id, Request $request ): \Illuminate\Http\JsonResponse {
        ...
        try {
            ...
            debug_log( "Vehicle Claim updated successfully!", $data );

            return api( $data )->success( 'Vehicle Claim Updated successfully!', Response::HTTP_CREATED );
        } catch ( \Exception $e ) {
            debug_log( "Vehicle Claim update failed!", $e->getTrace() );
            DB::rollback();
    ...
    public function destroy( $id ): \Illuminate\Http\JsonResponse {
        try {
            ...
            debug_log( "Vehicle Claim deleted successfully!", $data );

            return api( $data )->success( 'Vehicle Claim Deleted Successfully!' );
        } catch ( \Exception $e ) {
            debug_log( "Vehicle Claim deletion failed!", $e->getTrace() );
    ...
    public function downloadPhotos ( $id, Request $request )
    {
        try {
            ...
            $allImages = ClaimImage::where( ['claim_id' => $id, 'type' => $type] )->get();
    ...
    {
        $upload = app( FileManager::class )->upload( $request->file, 'uploads/vehicles/claims' );
        if ( ! $upload ) {
